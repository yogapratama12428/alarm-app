...
function App() {
  ...
  const { data, error, loading } = useFetching('https://fake-email-server.vercel.app/api/user')
  const handleToogle = () => setAddUser(!AddUser)

  const handleAddUser = async () => {
    await axios.post('https://fake-email-server.vercel.app/api/user', {
      nama: nama,
      email: email,
    }, {
      headers: {
  ...
  const handleDeleteUser = async (id) => {
    await axios.delete(`https://fake-email-server.vercel.app/api/user/${id}`)
      .then((response) => {
        console.log(response.data)
      })
      .catch((err) => {
  ...
  return (
    <>
      <div className="flex flex-no-wrap h-screen">
        ...
        <div className="container mx-auto py-10 h-64 md:w-4/5 w-11/12 px-6 flex flex-col gap-2">
          {/* Remove class [ border-dashed border-2 border-gray-300 ] to remove dotted border */}
          <button
            onClick={handleToogle}
            className="w-32 px-4 py-1 border rounded-lg bg-lime-500 font-bold text-white">
            Add
          </button>

          {
            AddUser && (
              <div className="flex w-52 flex-col gap-2 my-4 border rounded-md">
                <input
                  type="email"
                  id="UserEmail"
                  placeholder=" Email"
                  className="h-8 py-2 w-48 border-none bg-slate-200 rounded-md"
                    ...
                    data && (
                      data.map((d, index) => (
                        <tr key={d.id}>
                          ...
                          <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{d.nama}</td>
                          <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{d.email}</td>
